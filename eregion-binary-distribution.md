...
- [x] CI gera binaries multi-OS no tag (`.github/workflows/release.yml`)
- [x] Release sobe automaticamente os assets da matriz §3
- [x] `checksums.txt` (SHA-256) no mesmo release
- [x] Nomes de asset estáveis entre versões
- [x] `eregion version` / `--version` com protocol version (`protocol eregion/1`)
- [x] `eregion serve --config --manifest` funciona no asset publicado
- [x] Documentação do release aponta protocol `EREGION/1` (ou superior)

Local: `make dist` produz a mesma matriz em `dist/` com `checksums.txt`.
